...
export default function TableRow({ paramTodo }: { paramTodo: Todo }) {

  const [todo, setTodo] = useState<Todo>(paramTodo);
  ...
  return (
    <tr key={todo.id} className={`bg-white hover:bg-gray-100 ${todo.is_done ? 'text-gray-400 line-through pointer-events-none' : ''}`}>
      ...
      <td className="px-6 py-4">{todo.deadline ? new Date(todo.deadline).toLocaleDateString('ja-JP', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '-'}</td>
      ...
      <td className="px-6 py-4">{todo.assigned_person || '-'}</td>
      <td className="px-6 py-4 pointer-events-auto">
